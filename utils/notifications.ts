...
export async function notifyMemoryLeak(leak: MemoryLeak): Promise<void> {
  ...
  await showNotification(
    ...
    `"${leak.title}" is consuming ${leak.memoryHistory[leak.memoryHistory.length - 1].toFixed(
      1
    )} MB and growing.`,
    2
  );
...
export async function notifyTotalMemory(totalMemoryMB: number, tabCount: number): Promise<void> {
  ...
  await showNotification(
    ...
    `${tabCount} tabs are using a total of ${(totalMemoryMB / 1024).toFixed(
      1
    )} GB total memory. Consider closing some tabs.`,
    1
  );
}
