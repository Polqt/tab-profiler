import type { MLPrediction, TabMemoryInfo, UsagePattern } from '@/types';
import { getUsagePatterns } from './storage';
import { ML_WEIGHTS, ML_THRESHOLDS } from '@/constants/config';
...
export class TabUsagePredictor {
  ...
  async getPredictions(tabs: TabMemoryInfo[]): Promise<MLPrediction[]> {
    ...
    const predictions: MLPrediction[] = tabs.map((tab) => {
      ...
      // Weighted average
      const probability =
        recencyScore * ML_WEIGHTS.RECENCY +
        frequencyScore * ML_WEIGHTS.FREQUENCY +
        timePatternScore * ML_WEIGHTS.TIME_PATTERN +
        dayPatternScore * ML_WEIGHTS.DAY_PATTERN;

      // Determine confidence
      let confidence: 'high' | 'medium' | 'low';
      if (probability > ML_THRESHOLDS.HIGH_CONFIDENCE) confidence = 'high';
      else if (probability > ML_THRESHOLDS.MEDIUM_CONFIDENCE) confidence = 'medium';
      else confidence = 'low';
      ...
      return {
        ...
        title: tab.title,
        probability,
        suggestKeep: probability > ML_THRESHOLDS.KEEP_THRESHOLD,
        reasoning,
        confidence,
      };
    });
  ...
  private calculateRecencyScore(tab: TabMemoryInfo, now: number): number {
    ...
    return Math.exp(-ML_THRESHOLDS.DECAY_RATE * hoursSinceAccess);
  }
