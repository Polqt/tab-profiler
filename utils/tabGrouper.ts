import type { DomainGroup, TabMemoryInfo } from '@/types';
import { DOMAIN_HEALTH } from '@/constants/config';
...
export function groupTabsByDomain(tabs: TabMemoryInfo[]): DomainGroup[] {
  ...
  for (const [domain, domainTabs] of domainMap.entries()) {
    const totalMemory = domainTabs.reduce((sum, tab) => sum + tab.memoryUsageMB, 0);
...
export function getTopMemoryConsumers(tabs: TabMemoryInfo[], count: number = 5): TabMemoryInfo[] {
  return [...tabs].sort((a, b) => b.memoryUsageMB - a.memoryUsageMB).slice(0, count);
}
...
export function calculateDomainHealthScore(group: DomainGroup): number {
  ...
  if (group.totalMemoryUsageMB > DOMAIN_HEALTH.CRITICAL_MEMORY) {
    score -= 30;
  } else if (group.totalMemoryUsageMB > DOMAIN_HEALTH.HIGH_MEMORY) {
    score -= 20;
  } else if (group.totalMemoryUsageMB > DOMAIN_HEALTH.MEDIUM_MEMORY) {
    score -= 10;
  }

  if (group.tabCount > DOMAIN_HEALTH.HIGH_TAB_COUNT) {
    score -= 20;
  } else if (group.tabCount > DOMAIN_HEALTH.MEDIUM_TAB_COUNT) {
    score -= 10;
  }

  const avgMemory = group.totalMemoryUsageMB / group.tabCount;
  if (avgMemory > DOMAIN_HEALTH.HIGH_AVG_MEMORY) {
    score -= 15;
  }
